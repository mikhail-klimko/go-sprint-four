...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	spentCalories := float64((runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	speedInMsSquared := math.Pow(meanSpeed(action, duration)*kmhInMsec, 2)
	heightInM := height / cmInM
	weightFactor := walkingCaloriesWeightMultiplier*weight + speedInMsSquared/heightInM*walkingSpeedHeightMultiplier*weight
	spentCalories := float64(weightFactor * duration * minInH)
	return spentCalories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	spentCalories := float64((swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration)
	return spentCalories
}
